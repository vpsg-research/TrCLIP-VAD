...
def train(model, train_loader, test_loader, args, label_map: dict, device):
    ...
    for e in range(args.max_epoch):
        ...
        for i, item in enumerate(train_loader):
            step = 0
            visual_feat, text_labels, visual_feat_lengths, text, text_label, text_length = item
            visual_feat = visual_feat.to(device)
            visual_feat_lengths = visual_feat_lengths.to(device)
            text = text.to(device)
            text_labels = get_batch_label(text_labels, prompt_text, label_map).to(device)

            fusion_features, logits1, logits2 = model(visual_feat, text, None, prompt_text, visual_feat_lengths)

            loss1 = CLAS2(logits1, text_labels, visual_feat_lengths, device)
            loss_total1 += loss1.item()

            loss2 = CLASM(logits2, text_labels, visual_feat_lengths, device)
            loss_total2 += loss2.item()

            loss3 = torch.zeros(1).to(device)
            fusion_feature_normal = fusion_features[0] / fusion_features[0].norm(dim=-1, keepdim=True)
            for j in range(1, fusion_features.shape[0]):
                fusion_feature_abr = fusion_features[j] / fusion_features[j].norm(dim=-1, keepdim=True)
                loss3 += torch.abs(fusion_feature_normal @ fusion_feature_abr)
            loss3 = loss3 / 6
...
if __name__ == '__main__':
    ...
    train_dataset = XDDataset(args.visual_length, args.visual_train_list, args.text_train_list, False, label_map)
